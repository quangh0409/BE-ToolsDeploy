node {
  ...
  stage("Clone"){
   withCredentials([file(credentialsId: 'ssh_key_remote', variable: 'ssh_key_remote')]) {
        ...
        sh "ssh -o StrictHostKeyChecking=no -i ssh_id_rsa quang_vt204299@35.213.167.216 'cd BE-ToolsDeploy && git checkout dev)'"
    }
  }
  // stage("ScanSyntax"){
  //  withCredentials([file(credentialsId: 'ssh_key_remote', variable: 'ssh_key_remote')]) {
  ...
  stage("Clear"){
    withCredentials([file(credentialsId: 'ssh_key_remote', variable: 'ssh_key_remote')]) {
        sh "cat $ssh_key_remote > ssh_id_rsa"
        sh "chmod 400 ssh_id_rsa"
        // sh "ssh -o StrictHostKeyChecking=no -i ssh_id_rsa quang_vt204299@35.213.167.216 '(docker stop \$(docker ps -aq)  || echo no container) &&  (docker rmi -f \$(docker images -q) || echo no image) && docker builder prune -f'"
  ...
  stage("Deploy"){
    withCredentials([file(credentialsId: 'ssh_key_remote', variable: 'ssh_key_remote')]) {
        sh "cat $ssh_key_remote > ssh_id_rsa"
        sh "chmod 400 ssh_id_rsa"
        sh "ssh -o StrictHostKeyChecking=no -i ssh_id_rsa quang_vt204299@35.213.167.216 'cd BE-ToolsDeploy && docker compose -f ./docker-compose.yaml up --build -d'"
     }
  }
  // stage("Test"){
  //   withCredentials([file(credentialsId: 'ssh_key_remote', variable: 'ssh_key_remote')]) {
